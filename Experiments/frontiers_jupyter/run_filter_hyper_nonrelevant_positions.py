...
This file is the runfile for the filter_hyper_nonrelevant_positions step.
Deriving from our method, it possible for a non-hyper editing site to be recorded under the hyper-editing sites
category. Namely, for each pair of nucleotides X and Y that we perform the 3nt genome scheme,
other nucleotide mismatches than hyper X to Y or Y to X may be noted, enabled by the new conditions created by the
3nt scheme. Therefore, in this step, we filter the hyper editing files to include only X to Y or Y to X changes
"""
        ...
        for fastq in positive_fastqs:
            for node in graph_dict.keys():
                ...
                if os.path.isfile(dirstruct.pathName(fastq, node, Stages.editing_percent, need_suffix=True)):
                    all_pileup_files_from_last_stage.append(
