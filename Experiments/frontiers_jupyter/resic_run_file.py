...
class PipeTester():
        ...
        for fastq in positive_fastqs:
            for node in graph_dict.keys():
                ...
                if skip_existing_files and os.path.isfile(filtered_pileup_name):
                    logging.info(
                        f"SKIP pileup no change filtering for file {pileup_name} since {filtered_pileup_name} already exists")
                    continue

                # Todo I added this:
                if not os.path.isfile(pileup_name):
                    logging.info(
                        f"SKIP pileup no change filtering for file {pileup_name} since {pileup_name} was not created / does not exist")
                    continue
    ...
    def filter_readthreshold_test(self, threshold=1):
        """
        filters positions with less than a minimum number of reads
        :param threshold: the minimum threshold of reads (coverage) to keep a position
        :return:
        """
    ...
    def unique_site_test(self):
        ...
        for fastq in positive_fastqs:
            ...
            for node in graph_dict.keys():
                pileup_files_from_all_nodes.append(
                    dirstruct.pathName(fastq, node, Stages.editing_percent, need_suffix=True))
    ...
    def concensus_test(self, consensus_threshold=0.5):
        ...
        for node in graph_dict.keys():
            ...
            if skip_existing_files and \
                ...
                logging.info(f"SKIP pileup concensus filtering for file {node} since {filtered_pileups + [concensus_pileup]} already exists")
                continue
