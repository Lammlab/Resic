...
def stacked_bar(data, series_labels=None, category_labels=None,
                ...
                grid=True, reverse=False, y_limit=None, size_plot=None, use_dataframe=False, throw_zeros=False,
                dict_colors={}):
    ...
    use_dataframe   -- Bool, if true, data is treated as pandas df with series labels
     and category labels as rows and colums respectivly
    ...
    for row_index in range(len(data)):
        for column_index in range(len(data[row_index])):
            if data[row_index][column_index] != 0.0:
                data[row_index][column_index] = format(data[row_index][column_index] / sum_column[column_index] * 100,
                                                       '.2f')
