        ...
        with open(consensus_filename, 'w') as out:
            for pileuplist_list in parallel_gen:
                hits = sum(x is not None for x in
                           pileuplist_list)  # number of non-None lists, the number of pileups which have the entry
                if (hits / number_of_pileups) >= k:
                    pileup_list = next(
                        item for item in pileuplist_list if item is not None)  # first non-None pileup, all
