...
class SamLine:
	...
	def __repr__(self):
		...
		fields = [self.seq_id, self._flags, self.reference_id, self._gene_pos, self._mapq, self.cigar, self.rnext, self._pnext, self._tlen, self.sequence, self.quality]
...
def split_sam(sam_filename):
	...
	header_file = open(new_temp_file(),"w+") #tempfile.NamedTemporaryFile(prefix="sam_splitting", dir="/tmp", mode='w+', delete=False)
	aligned_file =  open(new_temp_file(),"w+") #tempfile.NamedTemporaryFile(prefix="sam_splitting", dir="/tmp", mode='w+', delete=False)
	misaligned_file =  open(new_temp_file(),"w+") #tempfile.NamedTemporaryFile(prefix="sam_splitting", dir="/tmp", mode='w+', delete=False)
